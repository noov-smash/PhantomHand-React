...
export const NintendoSwitchProCon = ({
  ...
  const calc = React.useCallback((val: number) => {
  ...
  const onLeftStickMove = React.useCallback(
    (e: MouseEvent) => {
      ...
      if (absX > absY) props.onTilt("LeftStickX", x);
      if (absX < absY) props.onTilt("LeftStickY", y);
    },
    [calc, props]
  );
  ...
  const onRightStickMove = React.useCallback(
    (e: MouseEvent) => {
      ...
      const absX = Math.abs(x);
      const absY = Math.abs(y);
      if (absX > absY) props.onTilt("RightStickX", x);
      if (absX < absY) props.onTilt("RightStickY", y);
    },
    [calc, props]
  );
